...
from protocol.keychron import ProtocolKeychron
...
class Keyboard(
    ...
    def __init__(self, dev, usb_send=hid_send):
        ...
        self.lighting_qmk_rgblight = self.lighting_qmk_backlight = (
            self.lighting_vialrgb
        ) = False

        # underglow
        self.underglow_brightness = self.underglow_effect = (
            self.underglow_effect_speed
        ) = -1
        self.underglow_color = (0, 0)
        # backlight
        self.backlight_brightness = self.backlight_effect = -1
        # vialrgb
        self.rgb_mode = self.rgb_speed = self.rgb_version = (
            self.rgb_maximum_brightness
        ) = -1
        self.rgb_hsv = (0, 0, 0)
    ...
    def reload(self, sideload_json=None):
        ...
        # Load Keychron-specific features (if supported)
        self.reload_keychron()

    def reload_layers(self):
        """Get how many layers the keyboard has"""

        self.layers = self.usb_send(
            self.dev, struct.pack("B", CMD_VIA_GET_LAYER_COUNT), retries=20
        )[1]

    def reload_via_protocol(self):
        data = self.usb_send(
            self.dev, struct.pack("B", CMD_VIA_GET_PROTOCOL_VERSION), retries=20
        )
        self.via_protocol = struct.unpack(">H", data[1:3])[0]

    def check_protocol_version(self):
        if (
            self.via_protocol not in SUPPORTED_VIA_PROTOCOL
            or self.vial_protocol not in SUPPORTED_VIAL_PROTOCOL
        ):
            raise ProtocolError()
    ...
    def reload_layout(self, sideload_json=None):
        ...
        if sideload_json is not None:
            ...
            data = self.usb_send(
                self.dev,
                struct.pack("BB", CMD_VIA_VIAL_PREFIX, CMD_VIAL_GET_SIZE),
                retries=20,
            )
            ...
            while sz > 0:
                data = self.usb_send(
                    self.dev,
                    struct.pack(
                        "<BBI", CMD_VIA_VIAL_PREFIX, CMD_VIAL_GET_DEFINITION, block
                    ),
                    retries=20,
                )
                if sz < MSG_LEN:
    ...
    def reload_keymap(self):
        ...
        for layer in range(self.layers):
            for row, col in self.rowcol.keys():
                if row >= self.rows or col >= self.cols:
                    raise RuntimeError(
                        "malformed vial.json, key references {},{} but matrix declares rows={} cols={}".format(
                            row, col, self.rows, self.cols
                        )
                    )
                # determine where this (layer, row, col) will be located in keymap array
                offset = (
                    layer * self.rows * self.cols * 2 + row * self.cols * 2 + col * 2
                )
                keycode = Keycode.serialize(
                    struct.unpack(">H", keymap[offset : offset + 2])[0]
                )
        ...
        for layer in range(self.layers):
            for idx in self.encoderpos:
                data = self.usb_send(
                    self.dev,
                    struct.pack(
                        "BBBB", CMD_VIA_VIAL_PREFIX, CMD_VIAL_GET_ENCODER, layer, idx
                    ),
                    retries=20,
                )
                self.encoder_layout[(layer, idx, 0)] = Keycode.serialize(
                    struct.unpack(">H", data[0:2])[0]
                )
                self.encoder_layout[(layer, idx, 1)] = Keycode.serialize(
        ...
        if self.layout_labels:
            data = self.usb_send(
                self.dev,
                struct.pack("BB", CMD_VIA_GET_KEYBOARD_VALUE, VIA_LAYOUT_OPTIONS),
                retries=20,
            )
    ...
    def reload_persistent_rgb(self):
        ...
        if "lighting" in self.definition:
            self.lighting_qmk_rgblight = self.definition["lighting"] in [
                "qmk_rgblight",
                "qmk_backlight_rgblight",
            ]
            self.lighting_qmk_backlight = self.definition["lighting"] in [
                "qmk_backlight",
                "qmk_backlight_rgblight",
            ]
            self.lighting_vialrgb = self.definition["lighting"] == "vialrgb"

        # If VialRGB is not declared but keyboard might support it, try to detect
        if not self.lighting_vialrgb:
            try:
                data = self.usb_send(
                    self.dev,
                    struct.pack("BB", CMD_VIA_LIGHTING_GET_VALUE, VIALRGB_GET_INFO),
                    retries=3,
                )[2:]
                version = data[0] | (data[1] << 8)
                if version == 1:
                    logging.info(
                        "VialRGB: Detected VialRGB support (not declared in JSON)"
                    )
                    self.lighting_vialrgb = True
            except Exception as e:
                logging.debug("VialRGB: Detection failed: %s", e)
        ...
        if self.lighting_vialrgb:
            ...
            if self.rgb_version != 1:
                raise RuntimeError(
                    "Unsupported VialRGB protocol ({}), update your Vial version to latest".format(
                        self.rgb_version
                    )
                )
    ...
    def reload_rgb(self):
        if self.lighting_qmk_rgblight:
            self.underglow_brightness = self.usb_send(
                self.dev,
                struct.pack(">BB", CMD_VIA_LIGHTING_GET_VALUE, QMK_RGBLIGHT_BRIGHTNESS),
                retries=20,
            )[2]
            self.underglow_effect = self.usb_send(
                self.dev,
                struct.pack(">BB", CMD_VIA_LIGHTING_GET_VALUE, QMK_RGBLIGHT_EFFECT),
                retries=20,
            )[2]
            self.underglow_effect_speed = self.usb_send(
                self.dev,
                struct.pack(
                    ">BB", CMD_VIA_LIGHTING_GET_VALUE, QMK_RGBLIGHT_EFFECT_SPEED
                ),
                retries=20,
            )[2]
            color = self.usb_send(
                self.dev,
                struct.pack(">BB", CMD_VIA_LIGHTING_GET_VALUE, QMK_RGBLIGHT_COLOR),
                retries=20,
            )[2:4]
            # hue, sat
        ...
        if self.lighting_qmk_backlight:
            self.backlight_brightness = self.usb_send(
                self.dev,
                struct.pack(
                    ">BB", CMD_VIA_LIGHTING_GET_VALUE, QMK_BACKLIGHT_BRIGHTNESS
                ),
                retries=20,
            )[2]
            self.backlight_effect = self.usb_send(
                self.dev,
                struct.pack(">BB", CMD_VIA_LIGHTING_GET_VALUE, QMK_BACKLIGHT_EFFECT),
                retries=20,
            )[2]
        ...
        if self.lighting_vialrgb:
            data = self.usb_send(
                self.dev,
                struct.pack("BB", CMD_VIA_LIGHTING_GET_VALUE, VIALRGB_GET_MODE),
                retries=20,
            )[2:]
    ...
    def reload_settings(self):
        ...
        while cur != 0xFFFF:
            data = self.usb_send(
                self.dev,
                struct.pack(
                    "<BBH", CMD_VIA_VIAL_PREFIX, CMD_VIAL_QMK_SETTINGS_QUERY, cur
                ),
                retries=20,
            )
            for x in range(0, len(data), 2):
        ...
        for qsid in self.supported_settings:
            ...
            data = self.usb_send(
                self.dev,
                struct.pack(
                    "<BBH", CMD_VIA_VIAL_PREFIX, CMD_VIAL_QMK_SETTINGS_GET, qsid
                ),
                retries=20,
            )
            if data[0] == 0:
    ...
    def set_layout_options(self, options):
        if self.layout_options != -1 and self.layout_options != options:
            ...
            self.usb_send(
                self.dev,
                struct.pack(
                    ">BBI", CMD_VIA_SET_KEYBOARD_VALUE, VIA_LAYOUT_OPTIONS, options
                ),
                retries=20,
            )
    ...
    def set_qmk_rgblight_brightness(self, value):
        ...
        self.usb_send(
            self.dev,
            struct.pack(
                ">BBB", CMD_VIA_LIGHTING_SET_VALUE, QMK_RGBLIGHT_BRIGHTNESS, value
            ),
            retries=20,
        )
    ...
    def set_qmk_rgblight_effect(self, index):
        ...
        self.usb_send(
            self.dev,
            struct.pack(">BBB", CMD_VIA_LIGHTING_SET_VALUE, QMK_RGBLIGHT_EFFECT, index),
            retries=20,
        )
    ...
    def set_qmk_rgblight_effect_speed(self, value):
        ...
        self.usb_send(
            self.dev,
            struct.pack(
                ">BBB", CMD_VIA_LIGHTING_SET_VALUE, QMK_RGBLIGHT_EFFECT_SPEED, value
            ),
            retries=20,
        )
    ...
    def set_qmk_rgblight_color(self, h, s, v):
        ...
        self.usb_send(
            self.dev,
            struct.pack(">BBBB", CMD_VIA_LIGHTING_SET_VALUE, QMK_RGBLIGHT_COLOR, h, s),
        )
    ...
    def set_qmk_backlight_brightness(self, value):
        ...
        self.usb_send(
            self.dev,
            struct.pack(
                ">BBB", CMD_VIA_LIGHTING_SET_VALUE, QMK_BACKLIGHT_BRIGHTNESS, value
            ),
        )
    ...
    def set_qmk_backlight_effect(self, value):
        ...
        self.usb_send(
            self.dev,
            struct.pack(
                ">BBB", CMD_VIA_LIGHTING_SET_VALUE, QMK_BACKLIGHT_EFFECT, value
            ),
        )
    ...
    def save_layout(self):
        ...
        for l in range(self.layers):
            ...
            for e in range(self.encoder_count):
                ...
                ccw = (l, e, 1)
                layer.append(
                    [self.encoder_layout.get(cw, -1), self.encoder_layout.get(ccw, -1)]
                )
    ...
    def restore_layout(self, data):
            ...
            for r, row in enumerate(layer):
                for c, code in enumerate(row):
                    if (l, r, c) in self.layout:
                        self.set_key(
                            l, r, c, Keycode.serialize(Keycode.deserialize(code))
                        )
        ...
        for l, layer in enumerate(data["encoder_layout"]):
            for e, encoder in enumerate(layer):
                self.set_encoder(
                    l, e, 0, Keycode.serialize(Keycode.deserialize(encoder[0]))
                )
                self.set_encoder(
                    l, e, 1, Keycode.serialize(Keycode.deserialize(encoder[1]))
                )
    ...
    def lock(self):
        ...
        self.usb_send(
            self.dev, struct.pack("BB", CMD_VIA_VIAL_PREFIX, CMD_VIAL_LOCK), retries=20
        )
    ...
    def qmk_settings_set(self, qsid, value):
        ...
        data = self.usb_send(
            self.dev,
            struct.pack("<BBH", CMD_VIA_VIAL_PREFIX, CMD_VIAL_QMK_SETTINGS_SET, qsid)
    ...
    def qmk_settings_reset(self):
        self.usb_send(
            self.dev,
            struct.pack("BB", CMD_VIA_VIAL_PREFIX, CMD_VIAL_QMK_SETTINGS_RESET),
        )
    ...
    def _vialrgb_set_mode(self):
        ...
        speed = getattr(self, "rgb_speed", None)
        if speed is None or not isinstance(speed, int) or speed < 0 or speed > 255:
            speed = 128

        hsv = getattr(self, "rgb_hsv", None)
        if hsv is None or not isinstance(hsv, (list, tuple)) or len(hsv) < 3:
            hsv = (0, 255, 255)
    ...
    def set_vialrgb_brightness(self, value):
        # Ensure rgb_hsv exists and is valid
        if not hasattr(self, "rgb_hsv") or self.rgb_hsv is None:
            self.rgb_hsv = (0, 255, value)
        else:
